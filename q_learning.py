...
def get_action_index(readout_t, epsilon, t):
    ...
    e_line = 1 - epsilon + epsilon/ACTIONS
    if t < OBSERVE:
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    img0,r_t,terminal = game_state.frame_step(a_t)
    s_t1 = [img0,s_t[0],s_t[1],s_t[2]]
...
def compute_target_q(next_state_batch, r_batch, readout_j1_batch, minibatch):
    ...
    """
    q_m = [np.max(d) for d in readout_j1_batch]
    target_q_batch = r_batch + GAMMA * q_m
