...
def discount_rewards(r):
    ...
    for idx in range(len(r)-1, -1, -1):
    	if r[idx] != 0:
    		rolling_sum = 0
    	rolling_sum = GAMMA * rolling_sum + r[idx]
    	discounted_r[idx] = rolling_sum
...
def compute_cost(readout,action_holder,reward_holder):
	...
	Returns:
		loss
	""" 
	# sum (log(readout[action]） * reward)
	prob_log = tf.reduce_sum(tf.multiply(tf.log(readout), action_holder), 1)
	loss = tf.multiply(prob_log, reward_holder)
	return loss
...
def get_action_index(readout_t,epsilon,t):
	...
	# else:
	# 	action_index = tf.argmax(readout_t)
	# return action_index
...
def trainNetwork(myAgent,sess):
	...
	while True:	
		...
		for i in range(0, K):
			...
			if (terminal):
				f = open('pg.txt', 'a')
				f.write("TimeStep," + str(t) + "; bar1_score," + str(bar1_score) + "; bar2_score," + str(bar2_score))
				f.close()
