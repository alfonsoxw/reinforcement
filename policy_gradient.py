...
def compute_cost(readout, action_holder, reward_holder):
    ...
    a = tf.multiply(readout,action_holder)
    b = tf.reduce_sum(a,axis = 1,keep_dims=True)
    loss = -1. *tf.log(b)*reward_holder
    '''
    taken_action = tf.argmax(action_holder,1)
    step_losses = np.zeros_like(reward_holder)
    print(readout.shape)

    step_losses = -1 * np.log(readout[i,taken_action[i]]) * reward_holder[i]
    loss = np.sum(step_losses)
    '''
    return loss
...
def trainNetwork(myAgent, sess):
    ...
    while True:
        ...
        for i in range(0, K):
            ...
            if (terminal):
                f = open("policy_gradient_result.txt", 'a')
