...
def get_action_index(readout_t,epsilon,t):
	...
	"""
	if (t < OBSERVE or np.random.uniform(0, 1) < epsilon):
...
def run_selected_action(a_t,s_t,game_state):
	...
	s_t1 = np.stack((x_t1, s_t[:, :, 0], s_t[:, :, 1], s_t[:, :, 2]), axis=2)
...
def compute_cost(target_q,a_t,q_value):
	...
	#current_q = tf.reduce_max(q_value*a_t,axis=1)
	current_q = tf.reduce_sum(tf.multiply(q_value, a_t), reduction_indices=1)

	# TO DO: Q-Learning Cost.
	cost = tf.reduce_mean(tf.square(target_q - current_q))
	return cost
...
def compute_target_q(next_state_batch,r_batch,readout_j1_batch,minibatch):
	...
	"""
	termi_batch = [d[4] for d in minibatch]
	target_q_batch = np.zeros(BATCH)
	for i in range(BATCH):
		if ~termi_batch[i]:
			target_q_batch[i] = r_batch[i] + GAMMA * np.max(readout_j1_batch[i, :])
		else:
...
def trainNetwork(s, readout, sess):
	...
	while True:
		...
		for i in range(0, K):
			...
			bar1_score = game_state.bar1_score
			bar2_score = game_state.bar2_score
			s_t1,r_t,terminal = run_selected_action(a_t,s_t,game_state)

			if(terminal) :
				f = open("q_learning_result.txt",'a')
				f.write(str(t) +","+str(bar1_score)+","+str(bar2_score)+"\n")
				f.close()
				break
